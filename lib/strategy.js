...
function Strategy(options, verify) {
  ...
  this._userAccountURL = options.userAccountURL || 'https://app.vssps.visualstudio.com/_apis/Accounts?api-version=3.2-preview';//ownerId=GUID
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    var accountOwnerURL = self._userAccountURL === 'https://app.vssps.visualstudio.com/_apis/Accounts?api-version=3.2-preview'
        ? self._userAccountURL + '&ownerId=' + json.id
        : self._userAccountURL;
    self._oauth2.get(accountOwnerURL, accessToken, function (err, body, res) {
      if (!err) {
        json.accounts = JSON.parse(body).value;
      }
